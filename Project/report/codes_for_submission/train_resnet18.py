...
def train(train_loader, model, criterion, optimizer, epoch):
    ...
    print("Current free GPU memory: {0} GB".format(f/1024/1024/1024))
    torch.cuda.empty_cache()
    t = torch.cuda.get_device_properties(0).total_memory
    r = torch.cuda.memory_reserved(0)
    a = torch.cuda.memory_allocated(0)
    f = r-a  # free inside reserved
    print("Free GPU memory after empty cache: {0} GB".format(f/1024/1024/1024))
...
def main():
    ...
    for epoch in range(params.start_epoch, params.epochs):
        ...
        if (epoch+1) % params.valfreq == 0 and epoch != 0:
            ...
            force_save = False
            if (epoch+1) % params.force_save and epoch != 0:
                force_save = True
