...
class CustomReLU(TorchFunc):
    ...
    def backward(self, y):
        ...
        grad_input = torch.addcmul(torch.zeros(y.size()),
                                   y, ((x > 0) & (y > 0)).type_as(y))
        return grad_input
        ##############################################################################
        #                             END OF YOUR CODE                               #
...
class GradCam:
    def guided_backprop(self, X_tensor, y_tensor, gc_model):
        """
        Compute a guided backprop visualization using gc_model for images X_tensor
        and labels y_tensor.
        ...
        correct_scores = out[range(X_tensor.size()[0]), y_tensor]

        correct_scores.backward(torch.ones(X_tensor.size()[0]))
        gbp_result = X_tensor.grad.data
        print(gbp_result.numpy().shape)
        gbp_result = gbp_result.numpy().transpose(0, 2, 3, 1)
    ...
    def grad_cam(self, X_tensor, y_tensor, gc_model):
        ...
        # Stores gradient of the module you chose above during a backwards pass.
        self.gradient_value = None
        # Stores the activation of the module you chose above during a forwards pass.
        ...
        correct_scores.backward(torch.ones(X_tensor.size()[0]))
        alpha = torch.sum(self.gradient_value, dim=[2, 3])
        cam = alpha.unsqueeze(2).unsqueeze(3) * self.activation_value
        cam = torch.relu(torch.sum(cam, dim=1))
