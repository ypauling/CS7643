...
class FoolingImage:
    def make_fooling_image(self, X, target_y, model):
        ...
        for it in range(max_iter):
            ...
            if it % 5 == 4:
                print("Current prediction: {}, score {}".format(
                    out.argmax(dim=1).item(), out.max(dim=1)[0].item()))
            if out.argmax(dim=1).item() == target_y:
                break

            correct_score = out[0, target_y]
            correct_score.backward()

            normalize_factor = X_fooling_var.data.norm()
            dX = learning_rate * X_fooling_var.grad.data / normalize_factor
