...
def style_transfer(name, content_image, style_image, image_size,
                   style_size, content_layer, content_weight, style_layers,
                   style_weights, tv_weight, content_loss, style_loss,
                   tv_loss, cnn, dtype, init_random=False):
    """
    Run style transfer!
    ...
    for t in range(200):
        ...
        loss_S = style_loss(feats, style_layers, style_targets, style_weights)
        loss_V = tv_loss(img_var, tv_weight)

        total_loss = loss_C + loss_S + loss_V
        total_loss.backward()
