...
X_tensor = torch.cat([preprocess(Image.fromarray(x))
                     for x in X], dim=0).requires_grad_(True)
...
# GradCam
# GradCAM. We have given you which module(=layer) that we need to capture gradients from,
# which you can see in conv_module variable below
gc_model = torchvision.models.squeezenet1_1(pretrained=True)
...
ggc_attr = compute_attributions(guided_gc, X_tensor, target=y_tensor)
visualize_attr_maps('visualization/guided_gradcam_captum.png', X, y,
                    class_names, [ggc_attr], ['Guided GradCAM'])

# Computing Guided BackProp
guided_bp = GuidedBackprop(model)
gbp_attr = compute_attributions(guided_bp, X_tensor, target=y_tensor)
visualize_attr_maps('visualization/guided_backprop_captum.png', X, y,
                    class_names, [gbp_attr], ['Guided Backpropagation'])
...
layer_gradcam_attr = compute_attributions(
    layer_gradcam, X_tensor, target=y_tensor, attribute_to_layer_input=False,
    relu_attributions=True)
visualize_attr_maps('visualization/layer_gradcam_captum.png', X, y,
                    class_names, [layer_gradcam_attr], ['Layer GradCAM'])

layer_conduct = LayerConductance(model, layer)
layer_conduct_attr = compute_attributions(
    layer_conduct, X_tensor, target=y_tensor)
layer_conduct_attr_mean = layer_conduct_attr.mean(axis=1, keepdim=True)
visualize_attr_maps('visualization/layer_conductance_captum.png', X, y,
                    class_names, [layer_conduct_attr_mean], ['Layer Conductance'])
