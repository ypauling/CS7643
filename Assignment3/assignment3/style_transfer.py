...
def content_loss_test(correct, content_loss):
    ...
    c_feats, content_img_var = features_from_img(
        content_image, image_size, cnn, dtype)

    bad_img = Variable(torch.zeros(*content_img_var.data.size()))
    feats = extract_features(bad_img, cnn)

    student_output = content_loss(
        content_weight, c_feats[content_layer], feats[content_layer]).data.numpy()
...
def style_loss_test(correct, style_loss):
    ...
    student_output = style_loss(
        c_feats, style_layers, style_targets, style_weights).data.numpy()
    error = rel_error(correct, student_output)
