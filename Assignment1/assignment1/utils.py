...
def load_mnist_trainval():
    ...
    split_point = int(num_train_data * 0.8)
...
def generate_batched_data(data, label, batch_size=32, shuffle=False, seed=None):
    ...
    num_data = len(data)
    while num_processed_data < num_data:
        tmp = num_data - num_processed_data
        num_rows = batch_size if batch_size < tmp else tmp
        cur_data = np.array(
            data[num_processed_data:(num_processed_data + num_rows)])
        cur_label = np.array(
            label[num_processed_data:(num_processed_data + num_rows)])
        batched_data.append(cur_data)
        batched_label.append(cur_label)
        num_processed_data += num_rows
...
def plot_curves(train_loss_history, train_acc_history, valid_loss_history, valid_acc_history):
    ...
    plt.title('Loss Plot')
    plt.legend()
    plt.savefig('{}loss{}.png'.format(folder, text))
    plt.close()

    plt.figure()
    x = list(range(len(train_acc_history)))
    plt.plot(x, train_acc_history, label='training accuracy')
    plt.plot(x, valid_acc_history, label='validation accuracy')
    plt.xlabel('# Epoch')
    plt.ylabel('Accuracy')
    plt.title('Accuracy Plot')
    plt.legend()
    plt.savefig('{}accuracy{}.png'.format(folder, text))
    plt.close()
