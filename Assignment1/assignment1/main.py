...
def run():
    ...
    for epoch in range(args.epochs):
        ...
        epoch_loss, epoch_acc = train(
            epoch, batched_train_data, batched_train_label, model, optimizer, args.debug)
        ...
        valid_loss, valid_acc = evaluate(
            batched_test_data, batched_test_label, model, args.debug)
        if args.debug:
            print(
                "* Validation Accuracy: {accuracy:.4f}".format(accuracy=valid_acc))
    ...
    if args.debug:
        print("Final Accuracy on Test Data: {accuracy:.4f}".format(
            accuracy=test_acc))
