...
class TestNetwork(unittest.TestCase):
    ...
    def test_two_layer_net(self):
        ...
        self.assertAlmostEqual(
            np.sum(np.abs(w1_grad_expected - model.gradients['W1'])), 0)
        self.assertAlmostEqual(
            np.sum(np.abs(b1_grad_expected - model.gradients['b1'])), 0)
        self.assertAlmostEqual(
            np.sum(np.abs(w2_grad_expected - model.gradients['W2'])), 0)
        self.assertAlmostEqual(
            np.sum(np.abs(b2_grad_expected - model.gradients['b2'])), 0)
