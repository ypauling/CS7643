...
class VanillaRNN(nn.Module):
    ...
    def __init__(self, input_size, hidden_size, output_size):
        ...
        #############################################################################
        self.hidden_unit = nn.Linear(input_size + hidden_size, hidden_size)
        self.output_unit = nn.Linear(input_size + hidden_size, output_size)
        self.tanh = nn.Tanh()
    ...
    def forward(self, input, hidden):
        ...
        #   going over one time step.
        #   Please refer to the structure in the notebook.                          #
        #############################################################################
        combined = torch.cat((input, hidden), dim=1)

        hidden = self.hidden_unit(combined)
        hidden = self.tanh(hidden)

        output = self.output_unit(combined)
