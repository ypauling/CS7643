    ...
    def forward(self, source, out_seq_len=None):
        """ The forward pass of the Seq2Seq model.
            Args:
                ...
                If None, the length is determined by the input sequences.
        """
        ...
        _, hidden_start = self.encoder(source)
        outputs = torch.zeros(
            (batch_size, out_seq_len, self.decoder.output_size))
        ...
        for t in range(0, out_seq_len):
            ...
            if t == 0:
                ...
                i_t = outputs[:, t - 1, :].topk(1, dim=-1)[1]

            o_t, hidden_start = self.decoder(i_t, hidden_start)
            outputs[:, t, :] = o_t
