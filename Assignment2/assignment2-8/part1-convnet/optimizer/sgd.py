    ...
    def update(self, model):
        ...
        for idx, m in enumerate(model.modules):
            if hasattr(m, 'weight'):
                ...
                #############################################################################
                velocity = self.grad_tracker[idx]["dw"] * self.momentum
                velocity -= self.learning_rate * m.dw
                m.weight += velocity
            ...
            if hasattr(m, 'bias'):
                ...
                #############################################################################
                velocity = self.grad_tracker[idx]["db"] * self.momentum
                velocity -= self.learning_rate * m.db
                m.bias += velocity
                self.grad_tracker[idx]["db"] = velocity
